...
# A number of functions/classes are adopted 
...
def _S_to_seq(S, mask):
    ...
    seq = [alphabet[c] for c, m in zip(S.tolist(), mask.tolist()) if m > 0]
    return ''.join(seq)
...
def tied_featurize(batch, device, chain_dict, conf):
    """ 
    ...
    """
    fixed_positions_dict= conf['fixed_positions_dict']
    omit_AA_dict = conf['omit_AA_dict']
    tied_positions_dict = conf['tied_positions_dict']
    pssm_dict = conf['pssm_dict']
    bias_by_res_dict = conf['bias_by_res_dict']
    ca_only = conf['ca_only']
        ...
        for step, letter in enumerate(all_chains):
            ...
            if letter in masked_chains:
                ...
                if fixed_positions_dict!=None:
                    fixed_pos_list = fixed_positions_dict[b['name']][letter]
                    if fixed_pos_list:
