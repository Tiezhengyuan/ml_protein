...
def download_pdb(pdb_id, dest_dir):
    ...
    lines = os.popen(wget_cmd).readlines()
...
def open_pdb( name ):
    ...
    if filename[-3:] == '.gz':
        lines = os.popen('zcat '+filename, 'r').readlines()
    else:
...
parser.add_option("--outdir", default='./', help="outdir")
...
if nres > 0:
    if not options.nopdbout:
        ...
        outfile = os.path.join(options.outdir, filename_stem + "_" + chainid + ".pdb")
    ...
    if not options.allchains:
        for chain in fastaseq:
            ...
            fastaid.write('\n')
            outfile = os.path.join(options.outdir, filename_stem+"_"+"".join(chain) + ".fasta")
        ...
        fastaid.write('\n')
        outfile = os.path.join(options.outdir, filename_stem+"_"+chainid + ".fasta")
        handle = open(outfile, 'w')
